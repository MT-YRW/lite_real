...
template <typename T = float> class InferenceNet {
public:
...
public:
  ...
  using BatchIOT = std::vector<T> &; // 改成一维
...
private:
  typename NetConfigT::InputConfigT
      input_cfg; // 这里编译的时候报错，必须要加typename，为什么？
  ...
  std::vector<T> interpolated_action; // 改成一维
...
InferenceNet<T>::InferenceNet(const std::string &ctrl_model_path,
                              ...
                              int interpolate)
    : async(async), interpolate(interpolate), policy_thread_running(false),
      runner_status(StatusT::STOPPED), input_cfg(net_config.input_config),
  ...
  //   }
  // }
  this->interpolated_action.resize(this->output_cfg.ctrl_model_output_size);
  // load config
...
template <typename T> void InferenceNet<T>::warm_up_net() { // 重写
...
template <typename T>
bool InferenceNet<T>::InferenceOnceErax(
    InputT clock, InputT command, InputT q, InputT dq, InputT last_action,
  ...
  torch::Tensor input_tensor =
      torch::cat({clk_tensor, cmd_tensor * this->input_cfg.scales_commands,
                  ...
                  base_proj_gravity_tensor *
                      this->input_cfg
                          .obs_scales_project_gravity}, // MT base_eu_tensor
                                                        // obs_scales_base_eular
                 0)
...
template <typename T>
typename InferenceNet<T>::StatusT
InferenceNet<T>::GetStatus() const { // 为什么需要加typename？
...
template <typename T> void InferenceNet<T>::perform_inference_erax() {
  ...
  // post inference 暂时不需要插值
  // if (this->interpolate > 0) {
  //   this->perform_interpolation();
  // }
  for (size_t i = 0; i < this->interpolated_action.size(); i++) {
    this->interpolated_action[i] =
        this->clip(this->scaled_action_tensor.item<float>(),
                   this->output_cfg.ctrl_action_lower_limit[i],
                   this->output_cfg.ctrl_action_upper_limit[i]);
  } // 这个维度和索引对的上吗？
...
//   this->logger->addLog(this->command_tensor[0].item<T>(),
//   "input_9_command_0");
//   this->logger->addLog(this->command_tensor[1].item<T>(),
//   "input_10_command_1");
...
//   // this->logger->addLog(this->clock_tensor[2].item<T>(),
//   "input_44_clock_2");
//   // this->logger->addLog(this->clock_tensor[3].item<T>(),
//   "input_45_clock_3");
